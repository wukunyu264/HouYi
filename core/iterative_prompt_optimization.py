...
class IterativePromptOptimizer:
    def __init__(
        ...
        self.max_concurrent_thread: int = 1
    ...
    def optimize(self):
        ...
        for iteration_num in range(self.iteration):
            ...
            if self.progress_callback:
                # 提取最佳 payload 和 分数 推送给 Celery
                best_payload = f"{self.best_chromosome.framework}{self.best_chromosome.separator}{self.best_chromosome.disruptor}"
                self.progress_callback(
                    iteration=iteration_num,
                    best_score=self.best_chromosome.fitness_score,
                    best_payload=best_payload,
                    llm_response=self.best_chromosome.llm_response
                )
